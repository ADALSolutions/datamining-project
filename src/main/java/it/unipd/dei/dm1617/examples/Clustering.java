...
public class Clustering implements Serializable
{
    ...
    public ArrayList<Point> getCentroids()
    {
        if(this.editCentroids)
        {
            ...
            {
                c.setEdit(true);//non proprio necessario ma per sicurezza
    ...
    public double kmeans()
    {
            ...
            for(int j = 0; j <= this.clusters.size() - 1; j++)
            {
                ...
                {
                    double dist=Math.pow(Distance.calculateDistance(centroid.parseVector(),points.get(l).parseVector()),2);
                    //System.out.println("A: "+points.get(l)+"    B: "+centroid+"    dist: "+dist);
                    sum+=dist;
                }
            }
            return sum;
